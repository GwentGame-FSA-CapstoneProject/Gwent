...
export default class SocketHandler {
  constructor(scene) {
    ...
    scene.socket.on("yourTurn", (socketId) => {
      ...
      if (socketId === scene.socket.id) scene.GameHandler.isMyTurn = true;
    });
    ...
    scene.socket.on("endRound", () => {
      ...
      let playerStr = gameHandler.totalStrength(
        gameHandler.playerClose,
        gameHandler.playerRange,
        gameHandler.playerSiege
      );
      let opponentStr = gameHandler.totalStrength(
        gameHandler.opponentClose,
        gameHandler.opponentRange,
        gameHandler.opponentSiege
      );
    ...
    scene.socket.on("cardPlayed", (cardName, socketId) => {
      //shows where opponent card goes
      if (socketId !== scene.socket.id) {
        let weather = [
          "biting_frost",
          "clear_weather",
        ...
        if (weather.includes(cardName)) {
          ...
          scene.DeckHandler.dealCard(
            ...
            cardName,
            "opponentCard"
          ).setCrop(0, 0, 300, 370).inPlay = true;
        }
      }
    });
  }
